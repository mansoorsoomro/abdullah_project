...
export default function Orders() {
    ...
    // Effect for Purchases Pagination
    useEffect(() => {
        if (user) {
            fetchOrders(user.id, purchasesPage);
        }
    }, [purchasesPage]);

    // Effect for Deposits Pagination
    useEffect(() => {
        if (user) {
            fetchPayments(user.id, depositsPage);
        }
    }, [depositsPage]);

    const fetchOrders = async (userId: string, page: number) => {
        try {
            const response = await fetch(`/api/orders/${userId}?page=${page}&limit=${itemsPerPage}`);
    ...
    const fetchPayments = async (userId: string, page: number) => {
        try {
            const response = await fetch(`/api/payments/${userId}?page=${page}&limit=${itemsPerPage}`);
            const data = await response.json();
            setPayments(data.payments || []);
            if (data.pagination) {
                setTotalDepositsPages(data.pagination.pages);
            }
        } catch (error) {
                    ...
                    >
                        ...
                        {totalPurchasesPages > 1 && (
                            <div className="p-4 flex justify-center gap-2 mt-8">
                                ...
                                <div className="flex items-center gap-2">
                                    {Array.from({ length: totalPurchasesPages }, (_, i) => i + 1)
                                        .slice(Math.max(0, purchasesPage - 3), Math.min(totalPurchasesPages, purchasesPage + 2))
                                        .map(pageNum => (
                                            <button
                                                key={pageNum}
                                                onClick={() => setPurchasesPage(pageNum)}
                                                className={`w-8 h-8 rounded flex items-center justify-center text-xs font-bold transition-colors ${purchasesPage === pageNum
                    ...
                    >
                        ...
                        {totalDepositsPages > 1 && (
                            <div className="p-4 flex justify-center gap-2 mt-8">
                                ...
                                <div className="flex items-center gap-2">
                                    {Array.from({ length: totalDepositsPages }, (_, i) => i + 1)
                                        .slice(Math.max(0, depositsPage - 3), Math.min(totalDepositsPages, depositsPage + 2))
                                        .map(pageNum => (
                                            <button
                                                key={pageNum}
                                                onClick={() => setDepositsPage(pageNum)}
                                                className={`w-8 h-8 rounded flex items-center justify-center text-xs font-bold transition-colors ${depositsPage === pageNum
                                                    ? 'bg-(--accent) text-black'
                                                    : 'bg-[#1a1a1a] text-gray-400 hover:text-white'
                                                    }`}
                                            >
                                ...
                                <button
                                    onClick={() => setDepositsPage(p => Math.min(totalDepositsPages, p + 1))}
